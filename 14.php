...
function createEqualsCost(callable $next, $value, $needle, $percent)
{
    return function (array $items) use ($next, $value, $needle, $percent) {
        return ($value == $needle) ? (1 - $percent / 100) * $next($items) : $next($items);
    };
}
...
$monthCost = createEqualsCost($priceBetweenCost, date('d'), 15, 5);
$birthdayCost = createEqualsCost($priceBetweenCost, date('m-d'), '08-12', 6);
